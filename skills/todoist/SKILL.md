...
# Local install in this workspace
npm install --prefix /data/.openclaw/workspace/skills/todoist-ts-cli todoist-ts-cli@^0.2.0
```
...
## Wrapper (workspace local)

Use the wrapper script so it picks up the local CLI and maps TODOIST_TOKEN → TODOIST_API_TOKEN when present:

```bash
/data/.openclaw/workspace/skills/todoist/scripts/todoist <args...>
```
